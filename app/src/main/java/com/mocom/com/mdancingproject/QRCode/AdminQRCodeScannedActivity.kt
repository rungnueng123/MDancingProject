...
import kotlinx.android.synthetic.main.activity_admin_qrcode_scanned.*
...
//            startActivity(intent)
    ...
    private fun resumeCamera() {
        Toast.LENGTH_LONG
        val handler = Handler()
        handler.postDelayed({ qrCodeScanner.resumeCameraPreview(this@AdminQRCodeScannedActivity) }, 2000)
    }

    companion object {
        private const val HUAWEI = "huawei"
        private const val MY_CAMERA_REQUEST_CODE = 6515
        fun getScanQrCodeActivity(callingClassContext: Context) = Intent(callingClassContext, AdminQRCodeScannedActivity::class.java)
    }
}
